...
def main():
    ...
    # TODO 2: liste des features Feast à récupérer
    features = [
    ...
    # TODO 7: run MLflow + fit + métriques + logs
    with mlflow.start_run(run_name=f"rf_baseline_{AS_OF}") as run:
        ...
        mlflow.log_param("random_state", rf_random_state)
        ...
        # Artefact JSON: cat_cols / num_cols
        mlflow.log_dict(
        ...
        mlflow.sklearn.log_model(
            sk_model=pipe,  # TODO 9 : on log la pipeline complète (prep + modèle), pas seulement clf
            artifact_path="model",
        ...
        # Pour aider ton rapport
        print(f"[INFO] Rows={len(df)} TrainTimeSec={train_time:.2f}")
