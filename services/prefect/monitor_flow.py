# services/prefect/monitor_flow.py
...
# Dates (TP2): month_000 => 2024-01-31, month_001 => 2024-02-29
...
def run_evidently(reference_df: pd.DataFrame, current_df: pd.DataFrame, as_of_ref: str, as_of_cur: str):
    ...
    # Seuil arbitraire pour ce TP : 30% des colonnes driftées => drift "significatif"
    DRIFT_SHARE_THRESHOLD = 0.3

    # Evidently : résumé + drift + drift sur une colonne (cible)
    metrics = [
        DataSummaryPreset(),
    ...
    if drift_share is None:
        # Fallback si Evidently change légèrement la structure
        drift_share = 0.0
...
def monitor_month_flow(
    ...
    # Drift sur la cible (si labels disponibles)
